...
sub ReadTasks {
	...
	if($_[2]<0) {
		...
		$datap = goah::Db::Tasks::Manager->get_tasks(\%dbsearch, sort_by => 'priority DESC, id DESC', limit => -1*$_[2]);
	} else {
		$datap = goah::Db::Tasks::Manager->get_tasks(\%dbsearch, sort_by => 'priority DESC, id DESC');
	}
